...
- [x] Cùng filesystem — chứng minh CẢ HAI CHIỀU. Chiều ĐỌC (2026-09-04): ghi `$HOME/lab/hello.txt` bằng `kubectl exec`, mở Theia bằng trình duyệt thật (Playwright) ⇒ Explorer hiện `/root/lab/hello.txt`, click vào thì tiêu đề đổi thành `hello.txt - root - Theia IDE`. Chiều GHI đóng 2026-09-08 trên `dlp-ide-config-edit` (layout `ide` thật): gõ chuỗi mốc duy nhất trong Theia ⇒ `cat` ở terminal cùng phiên thấy nó. **Đối chứng âm chạy TRƯỚC khi gõ** (marker vắng ở nền), và một marker thứ ba cùng hình dạng chưa từng gõ thì VẪN vắng — nên `grep` khớp đúng thứ nó định khớp. Bốn nguồn đọc độc lập: `cat` trong terminal · script chấm của chính bài (**Đạt**) · `kubectl exec` từ host (ngoài trình duyệt) · editor. Đo được kèm: Theia **tự lưu theo độ trễ** (marker gõ mà không Ctrl+S, không rời focus, vẫn lên đĩa) — `Ctrl+S` trong `step1.md` không sai nhưng không bắt buộc. [đo 2026-09-08](reports/harness/2026-09-08-ac-browser/run.md).
> ⛔ **Ô AC ĐÃ BỊ ĐẢO, KHÔNG PHẢI Ô CÒN MỞ** (2026-09-04). Bản gốc đòi *IDE không nghe `0.0.0.0` trong pod*. Ghim loopback buộc mọi byte IDE đi qua `portforward` của apiserver (hai pod = hai netns), mà apiserver cụm này đã restart 41 lần — tức lời hứa đó mua một chế độ hỏng đắt hơn thứ nó chặn. Chốt: **IDE nghe podIP, gateway nối thẳng**, và bảo đảm chuyển từ *cổng nào được mở* sang *ai chạm được cổng đó* — do NetworkPolicy gác, tức đúng ô `- [x]` ngay bên dưới (`Chỉ pod gateway chạm được sandbox:4000`).
>
> Giữ nguyên dạng ghi chú thay vì `- [ ]`: một ô tick không bao giờ đóng được sẽ nằm lại trong sổ sách mãi mãi và đọc ra thành *P6 chưa xong*.
...
- [~] CSP nới đúng một origin; chạy lại đối chứng CSP của 3.E, 0 vi phạm mới. — **HOÃN SANG P13 cùng 6.D, không phải nợ bỏ quên** (rà lại 2026-09-04). Ô này là **item 13 của task 6.D**, và 6.D đã được chủ dự án hoãn sang P13 ngày 2026-09-04 (xem đầu mục 6.D). Kiểm lại hiện trạng: `buildCsp` (`apps/web/src/server/security/headers.ts`) KHÔNG có `frame-src`, FE chưa có `<iframe>` IDE nào, và **chưa có origin thứ hai nào tồn tại để mà nới**. Nới CSP bây giờ là mở một lỗ cho một khung không tồn tại — đúng thứ mà chính ô này (qua `zero-violation-needs-negative-control`) sinh ra để ngăn. Đóng cùng lúc với iframe ở P13.
