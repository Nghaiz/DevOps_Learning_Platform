...
### Prerequisite 1.B0
- [ ] **Calico (đo từ lúc CÀI CRON, không phải từ lần restart gần nhất):** cron `rollout restart ds/calico-node` đã chạy liên tục **> 25h**, log cron có **≥ 2** lần restart thành công, và tạo pod mới trong `dlp-sandbox` thành công tại **T+25h** *và* **T+37h**. *Bản trước viết "> 25h kể từ lần deploy Calico gần nhất" — điều kiện đó **bất khả thi** một khi cron hoạt động, vì cron giữ "lần deploy gần nhất" luôn ≤ 12h. AC cũ tự vô hiệu hoá đúng lúc bản vá bắt đầu chạy.*
- [ ] Canary tạo-pod chạy trong cron/CI, và **đã thấy nó đỏ ít nhất một lần có chủ ý** (tắt cron 1 chu kỳ trên môi trường thử) — canary chưa từng đỏ là canary chưa biết có kêu hay không.
...
- [ ] **JWKS tới được từ gateway (1.B0.5):** `curl -s $GATEWAY_JWKS_URL | jq '.keys[0]'` trả `kty:"OKP"`, `crv:"Ed25519"`, và một `kid` không rỗng. 404 ở đây = plugin `jwt()` chưa mount ⇒ G2 vô nghĩa, dừng lại sửa trước.
