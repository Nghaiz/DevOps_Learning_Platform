...
> **Tiến độ:** ✅ **1.C-1 XONG (G1, G2, G3, G11, G13 + bước i của G8)** — 2026-08-10. Toàn bộ **chín bước kiểm trước upgrade** của contract §3 chạy thật; chủ hợp lệ nhận **101** + echo `dlp.terminal.v1`, mọi ca khác nhận đúng mã của bảng dưới. **G4–G10, G12 còn nợ** (cầu exec, bơm dữ liệu, resize, keepalive/extend, phần còn lại của rate-limit, metrics, 4 file `apps/web`).
>
...
- [ ] **Calico (đo từ lúc CÀI CRON, không phải từ lần restart gần nhất):** cron `rollout restart ds/calico-node` đã chạy liên tục **> 25h**, log cron có **≥ 2** lần restart thành công, và tạo pod mới trong `dlp-sandbox` thành công tại **T+25h** *và* **T+37h**. *Bản trước viết "> 25h kể từ lần deploy Calico gần nhất" — điều kiện đó **bất khả thi** một khi cron hoạt động, vì cron giữ "lần deploy gần nhất" luôn ≤ 12h. AC cũ tự vô hiệu hoá đúng lúc bản vá bắt đầu chạy.* ⏳ **ĐANG CHỜ** — cron tạo lúc `2026-08-09T07:40:14Z` (14:40 +07) ⇒ **T+25h = 2026-08-10 15:40 (+07)**, **T+37h = 2026-08-11 03:40 (+07)**. Canary 30 phút/lần tự tích luỹ bằng chứng: `kubectl -n dlp-sandbox get jobs -l app.kubernetes.io/component=cni-canary`.
...
> - [x] Đã đạt vế "≥ 2 lần restart thành công": 2 Job `dlp-calico-token-refresh` `Completed` (2026-08-09 ~20:00 và 2026-08-10 ~07:00 +07).
