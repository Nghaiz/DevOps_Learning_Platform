...
- [ ] Có traffic → `ExtendSession` được gọi; **chỉ ping/pong → `expires_at` KHÔNG đổi**; phiên im lặng tới khi hết `expiresAt` → WS đóng **`4404`**. *(Vế thứ ba SỬA 2026-08-12 ở 1.G-1: `4408` đã bị **bỏ khỏi contract §6**, xem ngay dưới.)*
...
- [ ] **Luật 5:** frame vượt read-limit → đóng đúng close code đã pin sau spike; bơm 5 MiB/s → `4429` và RSS gateway không tăng quá 2× baseline; **WS thứ 2 trên cùng session (trần 1, D17) → 429 + `code:"SESSION_IN_USE"`**; bão 200 resize/s → coalesce, **không** đóng, và `stty size` trong pod khớp **chính xác** giá trị cuối (không lệch 1 — status bar tmux đã tắt ở E4). ⏳ **BA TRÊN BỐN** (cập nhật 2026-08-12). ✅ Trần WS: `TestG13_TranMotWSChanDongThoiNhungKhongChanNoiLai` trên đường HTTP thật. ✅ Read-limit → **`1009`**: `TestFrameVuotReadLimitDongBang1009` (thêm ở 1.G-1 W3 — trước đó vế này **không có gì gác**). ✅ Byte-rate + RSS: đo trên cụm ở 1.G-2 M5 — `4429`, RSS 43.1→45.4 MiB dưới trần 2×. ⛔ **Vế thứ tư (bão 200 resize/s) KHÔNG đo được như viết vì nó mâu thuẫn với trần control 100/s của chính G8** — xem M5. Ô này chỉ tick được sau khi sửa câu chữ AC.
