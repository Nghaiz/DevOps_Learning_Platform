...
- [ ] `ReapSession` gọi 2 lần → cả hai OK; gọi với `user_id` người khác → từ chối, pod **vẫn sống**. ⏳ **NỬA ĐẦU ĐẠT trên cluster 2026-08-11** (dọn 2 phiên test của 1.F qua ĐÚNG đường `POST /api/trpc/session.reap`, không xoá pod bằng tay): lượt 1 trả `status=REAPED` cho cả hai; lượt 2 trên cùng `sessionId` **cũng trả OK** `status=REAPED`, không lỗi ⇒ idempotent. Trạng thái sau đó sạch: `pool:claimed` rỗng, `pool:quarantine` rỗng, chỉ còn đúng pod ấm, quota **1/10**. Hash `session:{id}` cố ý còn lại làm bia mộ cho `GetSession` nhưng **có TTL 251s** (không phải −1) nên không rò. **Nửa sau — `user_id` người khác bị từ chối và pod vẫn sống — CHƯA đo** (cần user thứ hai). Không tick cho tới khi đo vế đó.
