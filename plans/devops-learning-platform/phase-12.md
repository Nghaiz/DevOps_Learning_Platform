...
- [x] Ca mất pod giữa phiên: mã đóng đúng, câu tiếng Việt đúng, khe quota được trả. — **4/4, đo lại 2026-09-08 trên ảnh `p14a`** (`12e-pod-loss.mjs`): WS đóng `code=4404 reason="session gone"` sau 330ms, control frame cuối `SESSION_GONE`; câu tiếng Việt khớp nhánh 4404; Redis `session:{id}` ⇒ `FAILED` (không kẹt `ACTIVE`); quota `pods 3 → 4 → 3`, cpu `750m → 750m`. Bản vá là `podprobe.go` ở `4719d85` — gateway tra Redis TRƯỚC khi chọn mã, nên `exitCode=137` do xoá pod không còn bị đọc nhầm thành "người học tự gõ exit".
      Ghi lại trạng thái CŨ để khỏi tưởng ô này chưa từng đỏ: **3/4**. Câu tiếng Việt ✅, phiên không kẹt ✅, khe quota trả ✅. **Mã đóng SAI**: `1000 "exit"` thay vì `4404` do ĐUA giữa stream-đứt và sổ sách reaper (exitCode=137, gateway CÓ tra Redis nhưng Redis còn ghi phiên sống). Người bị thu hồi pod được báo "bạn đã tự gõ exit". Ngoài ownership P12 ⇒ để lại làm defect.
