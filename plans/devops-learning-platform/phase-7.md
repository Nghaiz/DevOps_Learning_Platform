...
- [x] `docs/k8s-in-pod.md` có số đo của **cả ba** đường + đối chứng pod-không-cluster. → bảng bốn cột (baseline / k3s / kind / vcluster) ở `docs/k8s-in-pod.md`: đối chứng chỉ-dockerd **94.89 MiB**, k3s **589 MiB**, kind **922.31 MiB**, vcluster **433 MiB**; đĩa 1 / 729 / 3553 MB. Phương pháp đo (cgroup mức pod trên host, `workingSet = memory.current − inactive_file`) ghi cùng chỗ. [report P7/P8](reports/2026-09-04-verify-p7-p8.md) §1.1.
- [x] Trong pod: `kubectl get nodes` → Ready, tạo được Deployment + ConfigMap + Service. → `kubectl exec p7-e2e -- dlp-k8s-wait 300` ⇒ **Ready sau 49 s**, `kubectl get nodes` trả node `v1.34.1+k3s1`; Deployment + ConfigMap + Service tạo được và **`nginx:1.29.0` KÉO ĐƯỢC trong cluster con** (`web-8645ff87cb-b5xql 1/1 Running`) — phép thử phân biệt *cluster lên* với *bài học chạy được*. [report P7/P8](reports/2026-09-04-verify-p7-p8.md) §1.2.
- [x] Câu hỏi "có cần `registry.k8s.io` không" được trả lời bằng **quan sát**, không bằng suy luận. → chạy dưới deny-all: `crictl ps` trong node container cho thấy `etcd`/`kube-apiserver`/`kube-controller-manager`/`kube-scheduler` đều `Running`, **tất cả từ image đã nằm sẵn trong image node**; không lượt kéo `registry.k8s.io` nào và không có gì treo. ⇒ **KHÔNG cần**, cho cả k3s lẫn kind. Giả thuyết trong plan đã trở thành quan sát — `docs/k8s-in-pod.md` §"mirror", [report P7/P8](reports/2026-09-04-verify-p7-p8.md) §1.1.
- [x] `requests`/`limits` profile K8s đặt theo **đỉnh** đo được; phép tính trần ghi trong values. → đo **dưới tải thật** (`dlp-k8s-broken-deploy`, 5 Deployment + Service + ConfigMap) trên **đường sản xuất**: 1 node **777.81 MiB**, 2 node **1094.79 MiB**. `777.81 > 768` ⇒ `requests` cũ nằm DƯỚI đỉnh thật, nên `k8s` → **1Gi**, `k8s-multinode` → **1536Mi** (biên ~31%, cùng tỉ lệ 589→768 đã dùng). Phép tính trần năm-ràng-buộc ghi trong `infra/helm/platform/values.yaml`. [report P7-bis](reports/2026-09-04-verify-p7bis-p9-debts.md) §1–§2.
...
- [x] `ckad-configmap-as-files`: verify **pass thật** ở bài đúng, **fail thật** ở bài sai (hai vế). → trên cụm thật: chưa làm gì ⇒ `❌ index.html content mismatch in ConfigMap.` **EXIT=1**; áp lời giải của `step1.md` ⇒ `✅ Verification successful!` **EXIT=0**. [report P7/P8](reports/2026-09-04-verify-p7-p8.md) §1.2.
- [x] `netpol-verify.sh` 22/22 với pod có cluster con; escape test tới apiserver chủ + metadata đều trượt. → `netpol-verify.sh` **22/22, LỆCH 0**, chạy khi trong namespace CÓ một pod đang chạy cluster con; `p7-escape-verify.sh` **9/9** gồm **4 đối chứng DƯƠNG** chạy TRƯỚC mọi phép *phải-trượt* (exec sống, DNS sống, mirror tới được, cluster con Ready) — thiếu chúng thì một pod chết cũng cho ra *cách ly hoàn hảo*. Với `NODES=2`: **13/13**, gồm phép thử từ pod GHIM TRÊN NODE 2. [report P7/P8](reports/2026-09-04-verify-p7-p8.md) §1.3, [report P7-bis](reports/2026-09-04-verify-p7bis-p9-debts.md) §3.
- [x] Bài `dlp-k8s-basics` (first-party) chạy end-to-end và qua cổng shellcheck `dlp-*` của CI. → 6 bước chạy **cả hai vế trên cụm thật** ([report P7/P8](reports/2026-09-04-verify-p7-p8.md) §1.2). Shellcheck: chạy lại 2026-09-04 trên **toàn bộ 39 script `content/**/dlp-*`** ở đúng ngưỡng CI (`severity: error`, xem `.github/workflows/ci.yml`) ⇒ **exit 0, 0 phát hiện**. ⚠ CI đang tạm dừng tới hết P14 (`docs/ci-paused.md`) nên đây là lượt chạy TAY cùng công cụ + cùng ngưỡng, không phải một lượt CI xanh.
