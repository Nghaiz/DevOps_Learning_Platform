...
\`get pods\` trông hệt như level 3: pod lên Running được một lúc rồi chết, số
RESTARTS tăng dần.

Nhưng lần này log ứng dụng **sạch**. Không exception, không stack trace, không
dòng nào báo lỗi. Dòng cuối cùng luôn là một dòng bình thường ở giữa công việc,
rồi đứt ngang.

Đó là dấu hiệu của một cái chết đến từ **bên ngoài tiến trình**. Ứng dụng không
tự thoát; nó bị giết, và không kịp ghi gì. Khi container vượt \`memory limit\`,
kernel giết nó ngay lập tức bằng tín hiệu không bắt được — không có cơ hội dọn
dẹp, không có cơ hội log.

Ba dấu vết phân biệt trường hợp này với một ứng dụng tự crash:

- Exit code **137** thay vì một mã do ứng dụng chọn.
- Lý do ghi ở **Last State**, không phải Current State — vì trạng thái hiện tại
  là container mới vừa được dựng lại.
- Log sạch cho tới đúng lúc đứt.

**Việc cần làm:** đưa \`xu-ly-anh\` về 3 replica chạy ổn định, không pod nào còn
mang lý do lỗi, và vẫn khai đầy đủ requests lẫn limits.

Node có 8Gi mỗi cái, nên bạn có chỗ. Nhưng đừng bỏ limit đi cho xong: container
không limit có thể ăn hết RAM của node và kéo theo mọi pod khác trên đó.`,
  ...
  hints: [
    ...
    'Exit code 137 là 128 + 9, tức tiến trình nhận SIGKILL. Với container trong Kubernetes, thủ phạm gần như luôn là OOM killer của kernel khi container chạm `memory limit`. Đối chiếu limit hiện tại với mức dùng thật qua `kubectl top pods -n noi-dung`.',
...
| Nguyên nhân | Dấu hiệu tách biệt |
|---|---|
| Ứng dụng tự chết (sai lệnh, sai cấu hình) | Log có exception hoặc thông báo lỗi; exit code do ứng dụng chọn |
| **OOMKilled** | Log **sạch** rồi đứt ngang; exit code **137**; chữ \`OOMKilled\` ở **Last State** |
| Liveness probe fail | Log sạch; Events ghi \`Liveness probe failed\`; RAM bình thường |

Điểm chung của hai loại sau: ứng dụng **không sai**. Nó bị giết từ bên ngoài.

Với OOM, kẻ giết là OOM killer của kernel khi container chạm \`memory limit\`.
Tín hiệu là SIGKILL — không bắt được, không có cơ hội dọn dẹp hay ghi log. Vì thế
dòng log cuối cùng luôn là một dòng bình thường ở giữa công việc.

Hai chi tiết hay bị bỏ qua: lý do nằm ở **Last State** chứ không phải State (State
là container mới vừa dựng lại), và \`memory limit\` là trần **cứng** — khác CPU,
nó không bóp cho chậm lại mà giết thẳng.`,
    cheatsheet: [
      ...
      { command: 'kubectl top pods -n <ns>', explain: 'Mức dùng bộ nhớ thật, để so với limit đang đặt.' },
      { command: 'kubectl get pods -n <ns> -o wide', explain: 'Cột RESTARTS và node đang chạy — nhiều pod cùng OOM trên một node là tín hiệu khác.' },
    ],
    takeaways: [
