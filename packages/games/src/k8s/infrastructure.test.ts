...
describe('interactive infrastructure', () => {
  ...
  it('creates and edits Node capacity in the scheduler index, without duplicate rendering', () => {
    const created = reduce(seed(), { gameId: 'k8s', tick: 0, kind: 'apply', yaml: yaml('Node', 'extra') });
    expect(created.accepted).toBe(true);
    expect(created.state.nodes).toHaveLength(2);
    const edited = reduce(created.state, { gameId: 'k8s', tick: 0, kind: 'edit', target: { kind: 'Node', name: 'host', namespace: '' }, yaml: yaml('Node', 'host') + 'spec:\n  capacity:\n    cpu: 6000\n    memory: 16384\n' });
  ...
  it('creates and removes Namespace in both API and namespace index', () => {
    const created = reduce(seed(), { gameId: 'k8s', tick: 0, kind: 'apply', yaml: yaml('Namespace', 'new-space') });
    expect(created.state.namespaces).toContain('new-space');
    const deleted = reduce(created.state, { gameId: 'k8s', tick: 0, kind: 'delete', target: { kind: 'Namespace', name: 'new-space', namespace: '' } });
  ...
  it('removes seeded Node from the scene and scheduler', () => {
    const deleted = reduce(seed(), { gameId: 'k8s', tick: 0, kind: 'delete', target: { kind: 'Node', name: 'host', namespace: '' } });
