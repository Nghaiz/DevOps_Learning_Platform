...
async function doiCheDo(page, che) {
  const nut = page.getByRole('group', { name: 'Chế độ vẽ' }).getByRole('button', { name: che });
  // Đã ở chế độ đó rồi thì bấm lại là vô hại nhưng tốn một nhịp chờ.
...
async function main() {
  ...
  for (const theme of ['light', 'dark']) {
    ...
    const coDark = await page.evaluate(() =>
    ...
    /*
     * Hai LƯỢT chụp, và lượt thứ hai mới là lượt quan trọng.
     *
     * Trước khi bấm "Chạy thử", MỌI node ở `pending` — tức chỉ một trong bảy hình
     * học của `STATE_ENCODING` được nhìn thấy. Sáu hình còn lại (`solid`,
     * `notched`, `ringed`, `ringed-double`, `hollow`, `sunken`) không xuất hiện ở
     * đâu cả, và "chúng biên dịch được" không có nghĩa là "chúng đọc được" — đó
     * đúng là bài học vừa phải trả giá với lỗi shader.
     *
     * ⚠ Ngay cả lượt sau cũng KHÔNG phủ hết bảy: `running`, `retrying` và
     * `queued` là trạng thái GIỮA CHỪNG, còn sau khi chạy xong mọi thực thể đã ở
     * trạng thái cuối. Nhìn được thêm `solid` / `notched` / `hollow`; ba hình kia
     * cần một thanh tua theo tick mà giao diện chưa có.
     */
    for (const pha of ['truoc-khi-chay', 'sau-khi-chay']) {
      if (pha === 'sau-khi-chay') {
      ...
      for (const che of ['2D', '3D']) {
        ...
        const ten = path.join(OUT, `cicd-${che.toLowerCase()}-${theme}-${pha}.png`);
