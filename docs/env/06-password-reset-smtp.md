...
**The 60-second clock starts at a replica's first request, not at boot.** Next runs `register()` on the first incoming request rather than at process start, so a replica that has served nothing has not begun sweeping. Measured 2026-09-13 on build `sm_yNBJEbf9NkB1GRCWkF`: a row with `expires_at` in the past survived 87 seconds of an idle `next start` with no log line, then a single `GET /login` produced `sweep: sent=0 failed=0 expired=1` and the row disappeared. Had `register()` run at boot, the 60-second tick would have cleared it at 60. The practical impact is small, since a web replica that serves no traffic is not carrying users either. It is recorded because "a live process means someone is sweeping" is the reading a person would use while tracing a message that never arrived, and that reading is wrong.
